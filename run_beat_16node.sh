...
# Generate 16-node hosts file
generate_hosts() {
    local hosts_file="/tmp/beat_hosts_16"
    for i in $(seq 0 15); do
        echo "${ALL_IPS[$i]}" >> "$hosts_file"
    done
    echo "$hosts_file"
}

# Generate keys for 16 nodes
generate_keys() {
    local ip=$1
    $SSH root@$ip "cd /root/BEAT/BEAT0 && \
        ...
        echo 'Keys generated'" 2>&1
}

# Deploy hosts file to all nodes
deploy_hosts() {
    local hosts_file=$1
    for ip in "${ALL_IPS[@]}"; do
        $SSH root@$ip "cp /root/BEAT/BEAT0/hosts /root/BEAT/BEAT0/hosts.bak 2>/dev/null; true"
        cat "$hosts_file" | $SSH root@$ip "cat > /root/BEAT/BEAT0/hosts"
    done
}

# Run BEAT on single node
run_beat_node() {
    local node_id=$1
    local ip=${ALL_IPS[$node_id]}
    local B=$2
    
    $SSH root@$ip "cd /root/BEAT/BEAT0 && \
        export LIBRARY_PATH=/usr/local/lib:\$LIBRARY_PATH && \
        export LD_LIBRARY_PATH=/usr/local/lib:\$LD_LIBRARY_PATH && \
        rm -f beat-node-${node_id}.log && \
...
check_node_done() {
    local node_id=$1
    local ip=${ALL_IPS[$node_id]}
    $SSH root@$ip "grep -c 'Consensus Finished' /root/BEAT/BEAT0/beat-node-${node_id}.log 2>/dev/null || echo 0" 2>&1
}
...
collect_result() {
    local node_id=$1
    local ip=${ALL_IPS[$node_id]}
    $SSH root@$ip "grep 'Total Time' /root/BEAT/BEAT0/beat-node-${node_id}.log 2>/dev/null | grep -oP 'Total Time: \K[0-9.]+' || echo ''" 2>&1
}
...
for ip in "${ALL_IPS[@]}"; do
    $SSH root@$ip "pkill -9 -f honest_party_test 2>/dev/null; pkill -9 -f BEAT 2>/dev/null; true" 2>/dev/null &
done
wait
sleep 3

# Step 2: Generate hosts file
echo "[Step 2] Generating hosts file..."
HOSTS_FILE=$(generate_hosts)
deploy_hosts "$HOSTS_FILE"

# Step 3: Generate keys (only on first node, then distribute)
echo "[Step 3] Generating threshold keys..."
generate_keys "${ALL_IPS[0]}"
sleep 2

# Step 4: Distribute keys to all nodes
echo "[Step 4] Distributing keys..."
...
# Step 5: Run experiments with different B values
B_LIST=(10 50 100 250 500 1000 2500 5000)
for B in "${B_LIST[@]}"; do
    echo ""
    echo "=== B=$B ==="

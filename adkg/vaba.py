...
class VABA:
    ...
    async def run_adkg(self, start_time):
        ...
        # 模拟交易作为 RBC0 输入，每笔 250 字节，总共识 B 笔，每节点 B/N 笔
        base_tx = (b'tx_node_' + str(self.my_id).encode() + b'_').ljust(250, b'\x00')
        tx_per_node = max(1, self.deg // self.n)
        tx_payload = base_tx * tx_per_node
        logging.info(f"RBC0 payload: {len(tx_payload)} bytes (total B={self.deg}, per node={tx_per_node})")
        await asyncio.create_task(self.make_rbc0(tx_payload))
        ...
        self.benchmark_logger.info(
            f"ADKG time:  {adkg_time:.8f} ACSS time: {acss_time:.8f}, RBC1 time: {self.rbc1_time:.8f}, Other time: {self.other_time:.8f},  RBC2 time: {self.rbc2_time:.8f}, Eval time: {self.eval_time+ self.eval2_time:.8f}, End view: {self.terminate_view}")

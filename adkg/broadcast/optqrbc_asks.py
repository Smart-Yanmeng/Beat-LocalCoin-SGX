...
async def optqrbc(sid, pid, n, f, leader, predicate, shares, input, output, send, receive):
    """
    ...
    if pid == leader:
        ...
        for i in range(n):
            shares[i].extend(m)
            send(i, (RBCMsgType.PROPOSE, bytes(shares[i])))
    ...
    while True:  # main receive loop
        try:
            ...
            if msg[0] == RBCMsgType.PROPOSE and leader_hash is None:
                ...
                valid, leader_msg = await predicate(leader_msg)
                if valid:
                ...
                if add_disperse_counter[my_stripe] >= f + 1 and not add_ready_sent:
                    add_ready_sent = True

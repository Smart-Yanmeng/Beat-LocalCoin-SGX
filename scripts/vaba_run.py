...
# 添加 StreamHandler 确保日志输出到 stdout
# 注意: LoggerAdapter 会传入 node_id extra，formatter 需要兼容
handler = logging.StreamHandler()
handler.setFormatter(logging.Formatter('%(message)s'))
# Monkey-patch format 以忽略 LoggerAdapter 注入的 extra 字段
_orig_format = handler.format
def _safe_format(record):
    if not hasattr(record, 'node_id'):
        record.node_id = ''
    return _orig_format(record)
handler.format = _safe_format
logger.handlers = [handler]  # 强制覆盖，确保 handler 存在

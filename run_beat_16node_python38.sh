#!/bin/bash
# 16-node BEAT distributed test with Python 3.8
PASS='York@233'
...
N=16
T=4  # N/4 for BEAT
RESULTS="/home/charlotte/MyWorkSpace/PROJECT/dumbo/beat_16node_results_python38.csv"

echo "╔══════════════════════════════════════════════════════════╗"
echo "║  16-node BEAT Distributed Test (Python 3.8)            ║"
echo "╠══════════════════════════════════════════════════════════╣"
echo "║  N=$N, T=$T                                            ║"
...
$SSH root@${ALL_IPS[0]} "cd /root/BEAT/BEAT0 && \
    ...
    python3.8 -m BEAT.commoncoin.prf_generate_keys $N $((T+1)) thsig${N}_${T}.keys && \
    python3.8 -m BEAT.threshenc.generate_keys $N $((N-2*T)) thenc${N}_${T}.keys && \

#!/bin/bash
# Distribute 16-node BEAT keys to all servers
PASS='York@233'
...
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  Distributing 16-node BEAT keys to all servers         ║"
echo "╚══════════════════════════════════════════════════════════╝"

# Distribute keys from node 0 to all other nodes
echo ""
echo "Distributing keys from node 0 (${ALL_IPS[0]})..."
...
for i in $(seq 0 15); do
    ip=${ALL_IPS[$i]}
    result=$($SSH root@$ip "ls -la /root/BEAT/BEAT0/thsig${N}_${T}.keys /root/BEAT/BEAT0/thenc${N}_${T}.keys /root/BEAT/BEAT0/ecdsa.keys 2>/dev/null | wc -l" 2>&1)
    if [ "$result" -eq 3 ]; then
        echo "  Node $i ($ip): ✅ OK"
    else
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  Keys distributed successfully!                        ║"
echo "║  Keys: thsig16_4.keys, thenc16_4.keys, ecdsa.keys    ║"
echo "╚══════════════════════════════════════════════════════════╝"
